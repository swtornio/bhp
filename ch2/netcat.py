...
def usage():
    print("BHP Net Tool")
    print("Usage: netcat.py -t target target_host -p port")
    print(
...
def main():
    ...
    if not listen and len(target) and port > 0:
        ...
        # this will block, so send ctl-d if not sending input
        # to stdin
...
def client_sender(buffer):
    ...
    try:
        ...
        print("Caught exception error: {}".format(exc))
...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        try:
            ...
            success_msg = "Successfully saved file to {}".format(upload_destination)
            client_socket.sendall(success_msg.encode('utf-8'))
        except:
            error_msg = "Failed to save file to {}".format(upload_destination)
